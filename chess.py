...
class Board(tk.Frame):
    ...
    def __init__(self, parent, length, width): #self=Frame, parent=root
        ...
        self.castled = False
        self.bot_enabled = True  # Enable simple bot
        self.set_squares()
            ...
            if self.buttons_pressed == 0: #stores square and button of first square selected
                ...
                if self.allowed_piece_move() and self.friendly_fire() == False: #makes sure the move is legal
                    ...
                    if  self.in_check() == True and self.castled == False: #for some reason it says king is in check after a castle so I set up a variable here that would prevent this code from running
                        ...
                        # Bot makes a move after player's turn
                        if self.bot_enabled and self.turns % 2 == 1:
                            self.after(500, self.bot_move)  # Small delay for better UX
        else:
            self.buttons_pressed = 0
            return

        #winsound.PlaySound('the rock meme sound.wav', winsound.SND_FILENAME)
    ...
    def get_simple_legal_moves(self, color):
        """Get simple legal moves for the given color (without complex check validation)"""
        legal_moves = []
        pieces = self.black_pieces if color == "black" else self.white_pieces
        
        # Store original values
        orig_sq1 = self.sq1
        orig_sq1_button = self.sq1_button
        orig_sq2 = self.sq2
        orig_sq2_button = self.sq2_button
        orig_piece_color = self.piece_color
        
        self.piece_color = color
        
        # Check every square for pieces of the given color
        for from_square in self.squares:
            from_button = self.squares[from_square]
            if from_button["image"] in pieces:
                self.sq1 = from_square
                self.sq1_button = from_button
                
                # Try moving to every other square
                for to_square in self.squares:
                    if from_square != to_square:
                        self.sq2 = to_square
                        self.sq2_button = self.squares[to_square]
                        
                        # Simple check - just basic piece movement and no friendly fire
                        try:
        ...
        self.sq1 = orig_sq1
        self.sq1_button = orig_sq1_button
        self.sq2 = orig_sq2
        self.sq2_button = orig_sq2_button
        self.piece_color = orig_piece_color
        
        return legal_moves

    def bot_move(self):
        """Make a random legal move for the bot (black)"""
        if self.turns % 2 != 1:  # Only move on black's turn
            return
            
        legal_moves = self.get_simple_legal_moves("black")
        ...
        # Store original piece on destination square
        orig_dest_piece = self.squares[to_square]["image"]
        
        # Execute the move directly
        piece_to_move = self.squares[from_square]["image"]
        self.squares[to_square].config(image=piece_to_move)
        self.squares[to_square].image = piece_to_move
        self.squares[from_square].config(image=self.white_images["blank.png"])
        self.squares[from_square].image = self.white_images["blank.png"]
        
        # Update castling flags
        if piece_to_move == "pyimage10":  # Black king
            self.bk_moved = True
        if piece_to_move == "pyimage14":  # Black rook
            if from_square == "a8":
                self.br1_moved = True
            elif from_square == "h8":
                self.br2_moved = True
        ...
        if piece_to_move == "pyimage12" and to_square.count("1") == 1:
            self.squares[to_square].config(image="pyimage13")  # Black queen
            self.squares[to_square].image = "pyimage13"
    ...
    def set_squares(self): #fills frame with buttons representing squares
        ...
        for x in range(8):
            for y in range(8):
                ...
                B = tk.Button(self, bg=self.square_color, activebackground="darkgreen")
    ...
    def import_pieces(self):
        ...
        for file in w_dirs:
            ...
            img = img.resize((80, 80), Image.ANTIALIAS if hasattr(Image, 'ANTIALIAS') else Image.BICUBIC)
            img = ImageTk.PhotoImage(image=img)
            self.white_images[file] = img
